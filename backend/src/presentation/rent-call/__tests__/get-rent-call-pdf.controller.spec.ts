...
describe('GetRentCallPdfController', () => {
  ...
  it('should use companyName for filename when tenant is company', async () => {
    ...
    expect(mockRes.set).toHaveBeenCalledWith(
      expect.objectContaining({
        'Content-Disposition': 'attachment; filename="appel-loyer-ACME Corp-2026-02.pdf"',
      }),
    );
  });
  ...
  it('should sanitize special characters in filename', async () => {
    ...
    expect(mockRes.set).toHaveBeenCalledWith(
      expect.objectContaining({
        'Content-Disposition': 'attachment; filename="appel-loyer-SCI _La Maison_Bleue_-2026-02.pdf"',
      }),
    );
  });
});
